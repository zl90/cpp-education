...
int main() {
  ...
  std::vector<Entry> phone_book = {
      {"john", 234234}, {"fred", 2345425}, {"tim", 345435}};
  ...
  const int g = a.foo; // Note: `a` doesn't contain the `bar` member as it has
                       // been "sliced" away.
  ...
  // Using hash maps:
  std::unordered_map<std::string, int> phone_book_hashmap = {
      {"john", 234234}, {"fred", 2345425}, {"tim", 345435}};
