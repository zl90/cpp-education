...
public:
  ...
  Vector<T> &
  operator=(const Vector<T> &obj) { // copy assignment operator override
    if (size > 0) {
      delete[] elements;
    }
    size = obj.size;
...
template <typename T>
T *begin(Vector<T> &x) { // iterator. This returns a pointer because the
                         // elements are allocated on the heap.
  return &x[0];
}

template <typename T> T *end(Vector<T> &x) { // iterator
  return &x[0] +
         x.get_size(); // point to one past the last element of the vector
}

template <typename T>
T *begin(const Vector<T> &x) { // iterator. This returns a pointer because the
                               // elements are allocated on the heap.
  return &x[0];
}

template <typename T> T *end(const Vector<T> &x) { // iterator
  return &x[0] +
         x.get_size(); // point to one past the last element of the vector
}
...
Vector<T> operator+(const Vector<T> &a,
                    const Vector<T> &b) { // operator overload
...
template <typename A>
concept Equality_comparable = requires(A a, A b) {
  { a == b } -> std::__boolean_testable;
  { a != b } -> std::__boolean_testable;
};

template <typename A, typename B = A>
concept Number = requires(A x, B y) {
  x + y;
  x - y;
  x *y;
  x / y;
...
int main() {
  ...
  for (auto &x : v4) // Use a ranged for loop over a Vector, thanks to the
  ...
  Vector<int> v7 = map(v6, [](const char &a) {
    int b = a;
    return b + 1;
  }); // Use a lambda as a functor in a higher order function.

  v7.print();
  // int sum_v7 = accumulate(begin(v7), end(v7), 0);

  std::string s = "Hello";
  // auto x2 = twice(s); // Causes error, string does not satisfy the Number
  // concept.
  int a = 44;
